...
_ssl_write (void *sd, char *data, size_t size, size_t * nbytes)
{
  ...
  while (rc <= 0
	 && (ec = SSL_get_error (sess->ssl, rc)) == SSL_ERROR_WANT_WRITE);
  if (rc > 0)
    {
      *nbytes = rc;
      return 0;
...
_ssl_read (void *sd, char *data, size_t size, size_t * nbytes)
{
  ...
  while (rc <= 0
	 && (ec = SSL_get_error (sess->ssl, rc)) == SSL_ERROR_WANT_READ);
  if (rc > 0)
    {
      *nbytes = rc;
      return 0;
...
start_ssl_client (NET_STREAM sd_server, const char *cafile, int verbose)
{
  ...
  stream_set_io (stream,
		 sd, _ssl_read, _ssl_write, _ssl_close, NULL, _ssl_strerror);
  return stream;
}
...
cipher_info (SSL * ssl_local)
{
  ...
  info (VERBOSE,
	ngettext ("Server public key is %d bit",
		  "Server public key is %d bits", bits), bits);
  ...
  if (!
      (X509_NAME_oneline
       (X509_get_subject_name (cert), x509buf, X509BUFSIZE)))
    {
  ...
  if (!
      (X509_NAME_oneline (X509_get_issuer_name (cert), x509buf, X509BUFSIZE)))
    {
...
rand_md5 (void)
{
  ...
  snprintf (buf, LINEBUFFER,
	    "%d%d%s", rand_int (32768), (int) getpid (),
	    (char *) getlogin ());
