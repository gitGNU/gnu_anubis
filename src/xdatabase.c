...
xupload()
{
	...
	while ((n = recvline(SERVER, remote_client, line,
			     sizeof(line) - 1)) > 0) {
		remcrlf(line);
	...
	if (!sec) {
		...
		swrite(SERVER, remote_client,
		       "450-Configuration update failed" CRLF);
		swrite(SERVER, remote_client, errmsg);
		swrite(SERVER, remote_client,
		       "450 Please fix and submit again" CRLF);
		unlink(tempname);
	} else {
...
xremove()
{
	...
	if (unlink(rcname) && errno != ENOENT) {
		anubis_error(SOFT,
			     ...
			     strerror(errno));
		swrite(SERVER, remote_client,
		       "450 Cannot unlink file" CRLF);
	}
	swrite(SERVER, remote_client,
	       "250 Configuration settings dropped" CRLF);
	free(rcname);
}
